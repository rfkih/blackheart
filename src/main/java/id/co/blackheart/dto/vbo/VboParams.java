...
public class VboParams implements Serializable {
    ...
    private boolean applyOverride(String key, Object raw) {
        switch (key) {
            case "compressionBbWidthPctMax" -> { BigDecimal v = bd(raw); if (v == null) return false; this.compressionBbWidthPctMax = v; }
            case "compressionAdxMax"        -> { BigDecimal v = bd(raw); if (v == null) return false; this.compressionAdxMax        = v; }
            case "requireKcSqueeze"         -> { Boolean b = bool(raw); if (b == null) return false; this.requireKcSqueeze          = b; }
            case "adxEntryMin"              -> { BigDecimal v = bd(raw); if (v == null) return false; this.adxEntryMin              = v; }
            case "adxEntryMax"              -> { BigDecimal v = bd(raw); if (v == null) return false; this.adxEntryMax              = v; }
            ...
            case "clvMin"                   -> { BigDecimal v = bd(raw); if (v == null) return false; this.clvMin                   = v; }
            case "clvMax"                   -> { BigDecimal v = bd(raw); if (v == null) return false; this.clvMax                   = v; }
            case "longRsiMax"               -> { BigDecimal v = bd(raw); if (v == null) return false; this.longRsiMax               = v; }
            case "shortRsiMin"              -> { BigDecimal v = bd(raw); if (v == null) return false; this.shortRsiMin              = v; }
            case "stopAtrBuffer"            -> { BigDecimal v = bd(raw); if (v == null) return false; this.stopAtrBuffer            = v; }
            case "maxEntryRiskPct"          -> { BigDecimal v = bd(raw); if (v == null) return false; this.maxEntryRiskPct          = v; }
            case "tp1R"                     -> { BigDecimal v = bd(raw); if (v == null) return false; this.tp1R                     = v; }
            case "breakEvenR"               -> { BigDecimal v = bd(raw); if (v == null) return false; this.breakEvenR               = v; }
            case "runnerBreakEvenR"         -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerBreakEvenR         = v; }
            case "runnerPhase2R"            -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerPhase2R            = v; }
            case "runnerPhase3R"            -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerPhase3R            = v; }
            case "runnerAtrPhase2"          -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerAtrPhase2          = v; }
            case "runnerAtrPhase3"          -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerAtrPhase3          = v; }
            case "runnerLockPhase2R"        -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerLockPhase2R        = v; }
            case "runnerLockPhase3R"        -> { BigDecimal v = bd(raw); if (v == null) return false; this.runnerLockPhase3R        = v; }
            case "minSignalScore"           -> { BigDecimal v = bd(raw); if (v == null) return false; this.minSignalScore           = v; }
