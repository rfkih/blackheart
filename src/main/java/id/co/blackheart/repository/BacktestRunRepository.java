...
public interface BacktestRunRepository extends JpaRepository<BacktestRun, UUID> {
    ...
    @Query(value = """
            ...
            WHERE (user_id = :userId OR triggered_by = 'RESEARCHER')
              ...
              AND (CAST(:strategyCode AS TEXT) IS NULL OR strategy_code ILIKE CONCAT(CAST(:strategyCode AS TEXT), '%') ESCAPE '\\')
              AND (CAST(:symbol AS TEXT) IS NULL OR asset ILIKE CONCAT(CAST(:symbol AS TEXT), '%') ESCAPE '\\')
