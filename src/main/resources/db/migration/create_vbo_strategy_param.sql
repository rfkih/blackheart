...
CREATE INDEX IF NOT EXISTS idx_vbo_param_overrides_gin
    ON vbo_strategy_param USING GIN (param_overrides);

CREATE OR REPLACE FUNCTION update_vbo_param_updated_time()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_vbo_param_updated_time
    BEFORE UPDATE ON vbo_strategy_param
    FOR EACH ROW
    EXECUTE FUNCTION update_vbo_param_updated_time();
